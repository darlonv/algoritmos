...
1. Elabore um registro chamado `Tempo`, possuindo dois campos: `data` e `horario`. O campo `data` é um registro, que possui dia, mês e ano, e `horario` é um outro registyro, que por sua vez possui os campos `hora`, `minuto` e `segundo`. Elabore também as seguintes funções:
    ...
    - Para calcular as diferenças de tempo, pesquise sobre as bibliotecas `Date` e `SimpleDateFormat`.
